...
function NurseUserDetails({selectedNurse}) {
  ...
  useEffect(() => {
    if (selectedNurse) {
      setFormData(prevState => ({
        ...prevState,
        partnerId: selectedNurse.PARTNER_ID
      }));
    }
  }, [selectedNurse]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3000/scheduleNurse', formData, { headers });
      ...
      <div className='doctor-details-wrapper flex flex-col md:flex-row gap-6 px-16 justify-center'>
          <div className='flex flex-col justify-between items-start py-4 gap-4 w-3/6 grow-0'>
            <div className="flex flex-row items-center w-full justify-between">
              <label htmlFor="selectedDoctor" className="mr-2 font-inter font-semibold text-sm">Selected Nurse:</label>
              <input id='selectedDoctor' type='text' name='selectedDoctor' className='w-4/6 h-10 bg-gray-200 text-sm rounded p-2 outline-none ' readOnly value= {selectedNurse.NURSE_SERVICE_NAME} />
            </div>
            <div className="flex flex-row items-center w-full justify-between">
...
export default NurseUserDetails;
