    ...
    <div className="Nav-component py-2 shadow-md fixed top-0 left-0 w-full bg-white z-50">
      <div className="Nav-container px-24 flex justify-between items-center">
        ...
        <div className="Nav-right-wrapper flex gap-10 items-center">
          ...
          <Link to=""><button className='Nav-button text-whistler-blue font-inter text-sm hover:text-whistler-green'>Help</button></Link>
          {isLoggedIn ? (
            <div className='flex gap-2 items-center'>
              <>
                <img src={accountCircle}className='w-10'/>
                <p className=' text-black p-2 text-sm'>Welcome {userFirstName}</p>
                <button className='Nav-login-button  px-4 py-2 bg-whistler-blue font-inter text-white rounded-md text-sm font-semibold hover:bg-whistler-green' onClick={handleLogout}>Logout</button>
              </>
            </div>
          ) : (
            <Link to="/login">
