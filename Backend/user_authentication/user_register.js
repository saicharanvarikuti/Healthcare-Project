...
router.post('/register', async (req, res) => {
  const userData = req.body;
    ...
    if (checkEmailResult[0][0].count > 0) {
      res.status(404).json({message:"Email already exists, please login"});
    } else {
      ...
      await database.promise().query('INSERT INTO USERS (FIRST_NAME, LAST_NAME, EMAILID, PASSWORD,STATUS) VALUES (?, ?, ?, ?, ?)', values);
